...
- 我的 HTTP 服务发送大响应导致机器带宽被占满，是哪一个客户端 ip 造成的？
...
的响应时间，ClaudeInsight 会将所有相同 `remote-ip` 的请求响应聚合，最终得出每个
`remote-ip` 的总耗时的相关指标。
...
claudeinsight 目前支持通过 `--metric` 指定的指标如下：
...
claudeinsight 目前支持通过 `--group-by` 指定的指标如下：
...
![ClaudeInsight stat quich result](/stat-quick-result.jpg)
...
```bash
./claudeinsight stat http --slow --time 10
```

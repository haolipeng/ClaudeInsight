...
目前还未支持，但后续有计划支持。([ISSUE-151](https://github.com/hengyoush/ClaudeInsight/issues/151))

## 支持运行在低版本内核上吗 ？

目前支持最低内核版本: 3.10.0-957，而且低版本内核下 claudeinsight 的会缺少某些功能。

目前 3.\*版本内核无法自动关联 NAT 前后的流量。

## 支持运行在 WSL 上的 Linux 吗 ?

理论上支持，但一般在 WSL 上的 Linux 发行版默认不带 linux
headers，但 claudeinsight 会依赖，所以可能需要修改编译选项手动编译内核，具体方法可参考：[Enabling eBPF/XDP for Kernel Tinkering on WSL2](https://dev.to/wiresurfer/unleash-the-forbidden-enabling-ebpfxdp-for-kernel-tinkering-on-wsl2-43fj)
...
这里手动下载和你的内核匹配的 BTF 文件，启动 claudeinsight 时通过 `--btf`
选项指定你下载的 btf 文件即可。

## 怎么理解 watch 结果中内核耗时的可视化部分 ?

![ClaudeInsight time detail](/faq-time-detail.png)
每一个方块代表数据包经过的节点，从上面依次看起，第一个是 Process，代表请求从进程发出到网卡设备，后边有一个(used: 0.02ms)代表从上一个节点到当前节点花费的时间。

然后请求从网卡发出，到网卡接收到响应（即为图中所示的向下箭头），之后就是响应接收的流程，从右往左。

响应数据复制到 TCP 缓存区，最后进程从缓冲区读取数据。

可以清楚的看到请求从进程发送到网卡，响应再从网卡复制到 Socket 缓冲区并且被进程读取的流程和每一个步骤的耗时。

## 运行 claudeinsight 后没有观察到 http 流量?

确认你想监控的协议不是 HTTP2，因为 claudeinsight 目前尚未支持。

## 为什么抓取到的请求数量明显少于真实的请求数量？
在大流量的情况下 claudeinsight 可能抓取不到完整的请求数据，检查 `/tmp` 目录下的 claudeinsight 日志，如果出现 `[dataReader] lost xx syscall data events` 的日志说明正是这种情况。
...
日志，如果有说明终端的颜色配置不正确，ClaudeInsight 需要 256 色的终端。  

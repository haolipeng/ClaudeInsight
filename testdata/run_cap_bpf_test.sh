...
sudo docker cp /host/claudeinsight/claudeinsight alpine:/
sudo docker cp ./testdata/test_not_add_cap_bpf.sh alpine:/
sudo docker cp /var/lib/claudeinsight/btf/current.btf alpine:/
sudo docker exec alpine sh -c 'sh /test_not_add_cap_bpf.sh "/claudeinsight --btf /current.btf"'
...
sudo docker cp /var/lib/claudeinsight/btf/current.btf alpine:/
sudo docker exec alpine sh -c 'sh /test_add_cap_bpf.sh "/claudeinsight --btf /current.btf"'
sudo docker stop alpine && sudo docker rm alpine
